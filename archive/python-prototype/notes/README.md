...
These notes explain how the Python prototype works, one file at a time. They were written for learning: to understand the code well enough to rebuild the important parts in TypeScript.

The Python code is archived, frozen at the git tag `python-prototype`. For what we're working on now, see [`PROJECT.md`](../../../PROJECT.md). The main lessons from this code are summarised there under "What the Python prototype taught us".
...
Tip: run headless mode alongside while you read. Run it from **inside the archive folder**, because the code looks for `config.json` in whichever folder you're in:
