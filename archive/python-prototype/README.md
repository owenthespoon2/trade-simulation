...
> **📦 Archived.** This Python prototype is finished and frozen at the git tag `python-prototype`. It was moved into `archive/python-prototype/` on 19 September 2026, and a TypeScript rewrite is next. For the current status and plan, see [`PROJECT.md`](../../PROJECT.md). Notes explaining how this code works are in [`notes/`](notes/README.md).
...
6.  Go into this folder first. The code looks for `config.json` in whichever folder you run it from:
    ```bash
    cd archive/python-prototype
    ```
7.  Run the main UI script:
    ```bash
    python ui_main.py
    ```
